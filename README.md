...
launches reuse the cached model. Settings selects the default target language
shown when a conversation opens. Gemma's audio mode detects the spoken source
language and transcribes each voice message once, then unloads; its transcript
and detected source language are stored with the persisted message before Gemma
reloads in text-only mode to translate. Later target-language changes translate
that saved text without processing the audio again. On Android, each result is spoken
