...
| --- | --- | --- | --- |
| Wi-Fi, BLE, and USB connection | `EdgezMeshSdk.listWifiNetworks`, `EdgezMeshSession.connectWifi`, `startBleScan`, `connectBle`, `connectUsb`, `disconnect` | [`app.dart`](../example/lib/src/app.dart), [`settings_tab.dart`](../example/lib/src/settings_tab.dart) | The Wi-Fi picker lists only `EdgeZ-*` SoftAPs and connects the selected network before opening its control socket; discovered BLE devices appear in `state.bleDevices`. |
