...
| Mesh status | `state.status`, `state.bleReady` | [`shared_widgets.dart`](../example/lib/src/shared_widgets.dart) | Includes link readiness, firmware version, and local MAC. |
...
| Device provisioning | `authorizeSession`, `requestDeviceSettings`, `sendDeviceSettings` | [`provisioning_screen.dart`](../example/lib/src/provisioning_screen.dart) | The example implements an eight-step flow. `authorizeSession` retains the empty compatibility handshake but does not gate on license status. |
